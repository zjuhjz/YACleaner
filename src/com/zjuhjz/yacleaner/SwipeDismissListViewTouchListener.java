...
public class SwipeDismissListViewTouchListener implements View.OnTouchListener {
    ...
    public AdapterView.OnItemLongClickListener makeLongclickListener(){
...
//                }
            ...
            case MotionEvent.ACTION_UP: {
                ...
                float deltaY = motionEvent.getRawY() - mDownX;
                mVelocityTracker.addMovement(motionEvent);
                ...
                /*
                 * add Math.abs(deltaX)>(Math.abs(deltaY)*2))
                 */
                if (Math.abs(deltaX) > mViewWidth / 2 && Math.abs(deltaX)>(Math.abs(deltaY)*2)&&deltaX > 0) {
                    dismiss = true;
                    Log.d(ProcessList.TAG, deltaX+"");
                    dismissRight = deltaX > 0;
                } 
                else if (mMinFlingVelocity <= velocityX && velocityX <= mMaxFlingVelocity
                        && (velocityY*2) < velocityX &&deltaX > 0) {
                    dismiss = true;
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (Math.abs(deltaX) > mSlop && Math.abs(deltaX)>(Math.abs(deltaY)*2) ) {
                	if(deltaX > 0){
                		mSwiping = true;
